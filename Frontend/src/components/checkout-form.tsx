...
export function CheckoutForm() {
  ...
  return <form onSubmit={submit} className="checkout-flow grid gap-8 lg:grid-cols-[1fr_420px]">
    ...
    <aside className="checkout-summary h-fit rounded-[1.7rem] bg-white p-5 shadow-[0_18px_70px_rgba(15,54,47,.09)] sm:p-7 lg:sticky lg:top-40">
      ...
      <div className="mt-6 max-h-[330px] overflow-y-auto pr-1">{cart.map((item, index) => { const authoritative = quotedItem(item.productId, item.variantId ?? null); const lineTotal = authoritative?.line_total ?? item.unitPrice * item.quantity; const itemError = itemErrors[index]; return <div id={`checkout-item-${index}`} key={item.key} className={`checkout-summary-line flex gap-3 border-b border-black/8 py-4 first:pt-0 ${itemError ? "has-error" : ""}`}><div className="relative h-20 w-17 shrink-0 overflow-hidden rounded-xl bg-[var(--mist)]"><AppImage src={item.image || undefined} alt={item.name} className="h-full w-full object-cover"/><span className="absolute right-1 top-1 grid h-5 min-w-5 place-items-center rounded-full bg-[var(--forest)] px-1 text-[10px] text-white">{item.quantity}</span></div><div className="min-w-0 flex-1"><p className="line-clamp-2 text-sm font-medium leading-5">{authoritative?.name || item.name}</p>{item.variantLabel ? <p className="mt-1 text-xs text-[var(--muted)]">{item.variantLabel}</p> : null}{itemError ? <p className="mt-2 text-xs font-semibold leading-5 text-[var(--clay)]" role="alert">{itemError}</p> : authoritative && item.quantity >= authoritative.available_stock ? <p className="mt-2 text-xs text-[var(--clay)]">Only {authoritative.available_stock} currently available</p> : null}</div><strong className="text-sm">{formatPrice(lineTotal)}</strong></div>; })}</div>
